...
// Store -.
func (r *TaskRepo) Store(ctx context.Context, task *entity.Task) error { //nolint:dupl // mirrors the standard repo insert pattern
...
func (r *TaskRepo) Update(ctx context.Context, task *entity.Task) error {
	sql, args, err := r.Builder.
		...
		Where(sq.Eq{columnID: task.ID, columnUserID: task.UserID}).
		ToSql()
	if err != nil {
...
func (r *TaskRepo) Delete(ctx context.Context, userID, taskID string) error {
	sql, args, err := r.Builder.
		Delete("tasks").
		Where(sq.Eq{columnID: taskID, columnUserID: userID}).
		ToSql()
	if err != nil {
