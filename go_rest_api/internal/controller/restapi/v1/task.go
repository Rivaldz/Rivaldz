...
// @Security    BearerAuth
// @Router      /tasks [post]
func (r *V1) createTask(ctx *fiber.Ctx) error { //nolint:dupl // mirrors the standard create handler template
	...
	if !r.bindBody(ctx, "createTask", &body) {
...
// @Security    BearerAuth
// @Router      /tasks/{id} [put]
func (r *V1) updateTask(ctx *fiber.Ctx) error { //nolint:dupl // mirrors the standard update handler template
	userID, ok := authUserID(ctx)
	if !ok {
		return unauthorizedResponse(ctx)
	}
	...
	if !r.bindBody(ctx, "updateTask", &body) {
		return badRequestResponse(ctx)
	}
