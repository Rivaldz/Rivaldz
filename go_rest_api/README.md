...
The template includes four domains to demonstrate multi-service architecture:
...
- **Blog** — posts and comments CRUD with transaction support

Auth, Tasks, and Translation are available across all four transports (REST, gRPC, AMQP RPC, NATS RPC).
Blog is implemented for the REST API.
...
The template includes four fully implemented domains.
...
Posts and comments CRUD with database transaction support.

| Operation    | REST                          | Notes                                        |
|--------------|-------------------------------|----------------------------------------------|
| Create post  | `POST /v1/posts`              | Requires auth                                 |
| List posts   | `GET /v1/posts`               | Pagination via `limit`/`offset`               |
| Get post     | `GET /v1/posts/:id`           | Any authenticated user                        |
| Update post  | `PUT /v1/posts/:id`           | Author only (403 otherwise)                   |
| Delete post  | `DELETE /v1/posts/:id`        | Author only; cascades comments in a transaction |
| Add comment  | `POST /v1/posts/:postID/comments` | Increments `posts.comment_count` atomically |
| List comments| `GET /v1/posts/:postID/comments` | Pagination via `limit`/`offset`            |
| Get comment  | `GET /v1/comments/:id`        | Any authenticated user                        |
| Update comment | `PUT /v1/comments/:id`      | Comment author only (403 otherwise)           |
| Delete comment | `DELETE /v1/comments/:id`   | Comment author only; decrements `comment_count` |

- Posts and comments are readable by any authenticated user, but only the owner can modify them.
- `posts.comment_count` is a denormalized counter maintained inside a `pgx.Tx` transaction so it never drifts from the real row count.
- Deleting a post removes all of its comments atomically (the FK has no `ON DELETE CASCADE`, so cleanup is done explicitly in a transaction).
- Input is validated with `go-playground/validator` (title max 255, body max 10000, comment max 2000).

## Quick start (Fresh Clone Guide)

Follow these steps to run the project after cloning it for the first time:

### 1. Initialize Your Module Name
Replace the template's module name with your own project's name:
```sh
make init-project MODULE=github.com/username/project-name
```
...
cp .env.example .env
```
Inside `.env`, configure your database URL (`PG_URL`), app URL (`APP_URL`), and **Feature Toggles** (e.g., turn off gRPC/RabbitMQ if you don't need them):
```env
ENABLE_REST=true
ENABLE_GRPC=false
...
### 3. Install Protobuf (If not installed)
This project requires `protoc` to compile the gRPC/Swagger schemas automatically.
For macOS users:
```sh
...
```
*Note: Ensure your `~/go/bin` is added to your terminal's `PATH`!*

### 4. Start Infrastructure (Docker)
Start the database (PostgreSQL) and any message brokers (RabbitMQ/NATS):
```sh
make compose-up
```
...
```sh
# For Development with Hot-Reload (Air):
make dev

# For Standard Execution (with auto-migration):
make run
```
...
Tests are written at two levels: **unit tests** (business logic with mocks) and **integration tests** (full stack
over HTTP/gRPC/RMQ/NATS inside Docker).
...
make test
# or
go test -race -covermode atomic -coverprofile=coverage.txt ./internal/... ./pkg/...
```

| Package                          | Coverage | Notes                                    |
|----------------------------------|----------|------------------------------------------|
| `internal/entity`                | ~73%     | Validation & state-machine methods       |
| `internal/usecase`               | 95%*     | Task/User/Translation + **Post/Comment** with generated mocks |
| `internal/usecase/post`          | ~98%*    | Create/Get/List/Update/Delete + ownership 403 |
| `internal/usecase/comment`       | ~98%*    | Create/Get/ListByPost/Update/Delete + ownership 403 |
| `pkg/jwt`                        | 81%      | Token generate/parse                     |
| `pkg/logger`                     | 100%     | Structured logging                       |
| `internal/controller/restapi/middleware` | 31% | Auth middleware                    |

\* Measured with `-coverpkg=./internal/...` because the tests live in an external `usecase_test` package.

The Blog use case is exercised by `internal/usecase/post_test.go` and `internal/usecase/comment_test.go`
(30+ cases: create/get/list/update/delete, 403 ownership, 404, repo error propagation).
...
This boots Postgres + RabbitMQ + NATS + the app, runs migrations, then executes `integration-test/` inside a
container. All suites pass:

- Auth: register/login/profile over HTTP, gRPC, RMQ, NATS
- Tasks: CRUD + status transitions over HTTP, gRPC, RMQ, NATS
- Translation: do-translate + history over HTTP, gRPC, RMQ, NATS
- **Blog: posts & comments CRUD, 403 authorization, 400/401/404 errors, comment_count consistency** (`integration-test/blog_test.go`)

### Manual smoke test (blog)

With the stack running, a quick end-to-end check can be done with `curl` — see [vibing/test_e2e.md](vibing/test_e2e.md)
for the full scenario matrix and direct DB verification queries.

## Known Limitations & Future Improvements

### Limitations

- **`comment_count` denormalization** — the counter is kept consistent inside transactions, but a crash between
  the comment insert and the counter update (or an `UPDATE` race on the same post) could theoretically desync it.
  A nightly reconciliation job or a trigger would fully close this gap.
- **Blog domain is REST-only** — unlike Auth/Tasks/Translation, the blog handlers are not exposed over
  gRPC/AMQP/NATS RPC.
- **Offset pagination** — `limit`/`offset` can be slow on large datasets; a cursor-based approach is preferable
  at scale.
- **No soft delete / audit log** — deleting a post permanently removes comments; no history is kept.
- **Integration-test idempotency** — the Go integration tests register users named after the test case, so they
  must run against a fresh database (`docker compose down -v`) between repeated runs.
- **Swagger regeneration is manual** — run `make swag-v1` after changing handlers.
- **Password hashing cost** is fixed at `bcrypt.DefaultCost`; no per-deployment tuning knob.

### Future improvements

- Soft delete (`deleted_at`) and audit trail for posts/comments.
- Nested comments (replies) with a self-referencing `parent_id`.
- Cursor-based pagination.
- Rate limiting and per-user quotas.
- Redis caching for hot `GET /posts` reads.
- Distributed tracing (OpenTelemetry).
- Expose blog over gRPC/AMQP/NATS to match the other domains.
